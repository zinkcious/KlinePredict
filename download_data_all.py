...
def get_all_stock(market):
    url = "http://10.15.144.131/market/static"
    ...
    resp = requests.get(url=url, params=params)
    result = resp.json()["Data"]["RepDataStatic"]
...
if __name__ == '__main__':
    stock_list_SH = get_all_stock(market='SH')
    # print(len(stock_list_SH))
    stock_list_SZ = get_all_stock(market='SZ')
    # print(len(stock_list_SZ))
    # SH 2311
    # SZ 2887

    stock_all = stock_list_SH + stock_list_SZ

    stock_list = get_stock()

    stock_list_ = [s for s in stock_all if s not in stock_list]
    print(len(stock_list_))
    ...
    with ProcessPoolExecutor(max_workers=3) as executor:
        executor.map(get_data, iter(stock_list_), chunksize=4)
